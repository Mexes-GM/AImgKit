...
def _kuwahara_rgb_uint8(orig_img: np.ndarray, radius: int,
    ...
    avgs = np.empty((4, *image.shape), dtype=image.dtype)            # (4,H,W,3)
    stddevs = np.empty((4, *image.shape[:2]), dtype=image.dtype)     # (4,H,W)
    ...
    filtered = np.take_along_axis(avgs, indices[None, ..., None], 0)
